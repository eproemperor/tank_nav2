  ...
  BooleanType isSparse)
{
  ...
  if (guessedConstraints != 0) {
    for (int_t i = 0; i < nC; i++) {
      if (isEqual(guessedConstraints[i], -1.0) == BT_TRUE) {
        ...
        char msg[MAX_STRING_LENGTH];
        snprintf(
          msg, MAX_STRING_LENGTH, "ERROR (qpOASES): Only {-1, 0, 1} allowed for status of constraints!");
        myMexErrMsgTxt(msg);
        return -1;
      }
    }
  ...
  for (int_t k = 0; k < nP; ++k) {
    if (k > 0) {
      ...
      maxCpuTimeOut = (maxCpuTimeIn >= 0.0) ? maxCpuTimeIn : INFTY;
      returnvalue =
        QP->hotstart(g_current, lb_current, ub_current, lbA_current, ubA_current, nWSRout, &maxCpuTimeOut);
    }
...
void mexFunction(int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[])
{
  ...
  nP = (int_t)mxGetN(prhs[g_idx]); /* number of columns of the gradient matrix (vectors series have to be
                                      stored columnwise!) */
  ...
  if (isSimplyBoundedQp == BT_TRUE) {
    ...
    if ((!mxIsEmpty(prhs[4])) && (mxIsStruct(prhs[4]))) {
      myMexErrMsgTxt("ERROR (qpOASES): Fifth input argument must not be a struct when solving QP with "
                     "general constraints!\nType 'help qpOASES' for further information.");
      return;
    }
  ...
  if (auxInput_idx >= 0)
    setupAuxiliaryInputs(
      prhs[auxInput_idx], nV, nC, &hessianType, &x0, &guessedBounds, &guessedConstraints, &R_for);
