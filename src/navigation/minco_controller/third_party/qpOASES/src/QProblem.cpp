  ...
  const Constraints * const guessedConstraints)
{
  ...
  if (options.enableFarBounds == BT_FALSE) {
    ...
    for (;;) {
      ...
      /* Automatically call standard solveQP if regularisation is not active. */
      returnvalue = solveRegularisedQP(g_new,
        lb_new_far,
        ub_new_far,
        lbA_new_far,
        ubA_new_far,
  ...
  const Constraints * const guessedConstraints)
{
  ...
  returnvalue = loadQPvectorsFromFile(
    g_file, lb_file, ub_file, lbA_file, ubA_file, g_new, lb_new, ub_new, lbA_new, ubA_new);
  if (returnvalue != SUCCESSFUL_RETURN) {
    if (ubA_file != 0)
      delete[] ubA_new;
    if (lbA_file != 0)
  ...
  returnvalue =
    hotstart(g_new, lb_new, ub_new, lbA_new, ubA_new, nWSR, cputime, guessedBounds, guessedConstraints);
  ...
  real_t * const cputime)
{
  ...
  if (obtainAuxiliaryWorkingSet(
        xOpt, yOpt, guessedBounds, guessedConstraints, &auxiliaryBounds, &auxiliaryConstraints) !=
      SUCCESSFUL_RETURN)
    return THROWERROR(RET_INIT_FAILED);
  ...
  BooleanType isFirstCall)
{
  ...
  status = QPS_PERFORMINGHOMOTOPY;  // AW TODO: Not sure if this is too early, but otherwise removeBounds
                                    // will complain
  ...
  */
  ...
  for (iter = nWSRperformed; iter < nWSR; ++iter) {
    ...
    returnvalue = determineDataShift(g_new,
      lbA_new,
      ubA_new,
      lb_new,
      ub_new,
    ...
    returnvalue = performStep(delta_g,
      delta_lbA,
      delta_ubA,
      delta_lb,
      delta_ub,
  ...
  int_t number, BooleanType updateCholesky, BooleanType allowFlipping, BooleanType ensureNZC)
{
  ...
  if (number_idx < nAC - 1) {
    ...
    for (j = (nAC - 2 - number_idx); j >= 0; --j) {
      computeGivens(TT(nAC - 2 - j, tcol + 1 + j),
        TT(nAC - 2 - j, tcol + j),
        TT(nAC - 2 - j, tcol + 1 + j),
        TT(nAC - 2 - j, tcol + j),
        c,
  ...
  int_t number, BooleanType updateCholesky, BooleanType allowFlipping, BooleanType ensureNZC)
{
  ...
  if (nAC > 0) {
    ...
    for (j = (nAC - 1); j >= 0; --j) {
      computeGivens(
        tmp[nAC - 1 - j], TT(nAC - 1 - j, tcol + j), TT(nAC - 1 - j, tcol + j), tmp[nAC - 1 - j], c, s);
      nu = s / (1.0 + c);
  ...
  SubjectToStatus & addStatus)
{
  ...
  if (removeBoundNotConstraint) {
    ...
    returnvalue = determineStepDirection(
      nul, nul, nul, ek, ek, BT_FALSE, BT_FALSE, delta_xFX, delta_xFR, delta_yAC, delta_yFX);
    delete[] ek;
    delete[] nul;
  } else {
    ...
    returnvalue = determineStepDirection(
      nul, ek, ek, nul, nul, BT_FALSE, BT_TRUE, delta_xFX, delta_xFR, delta_yAC, delta_yFX);
    delete[] ek;
    delete[] nul;
  }
  ...
  if (normXi < options.epsNZCTests * normS) {
    ...
    performPlainRatioTest(
      nFR, FR_idx, x_W, delta_xFR, options.epsNum, options.epsDen, sigmaUBnd, addUBndIdx);
    ...
    addLBndIdx = -1;
    performPlainRatioTest(
      nFR, FR_idx, x_W, delta_xFR, options.epsNum, options.epsDen, sigmaLBnd, addLBndIdx);
    ...
    addUCnstrIdx = -1;
    performPlainRatioTest(
      nIAC, IAC_idx, Ax_W, As, options.epsNum, options.epsDen, sigmaUCnstr, addUCnstrIdx);
    if (removeBoundNotConstraint == BT_FALSE && constraints.getStatus(remIdx) == ST_LOWER) {
      ...
      real_t one = 1.0;
      performPlainRatioTest(
        1, &remIdx, &Ax_u[remIdx], &one, options.epsNum, options.epsDen, sigmaUCnstr, addUCnstrIdx);
    }
    ...
    addLCnstrIdx = -1;
    performPlainRatioTest(
      nIAC, IAC_idx, Ax_W, As, options.epsNum, options.epsDen, sigmaLCnstr, addLCnstrIdx);
    if (removeBoundNotConstraint == BT_FALSE && constraints.getStatus(remIdx) == ST_UPPER) {
      /* also consider constraint which is to be removed */
      real_t one = 1.0;
      performPlainRatioTest(
        1, &remIdx, &Ax_l[remIdx], &one, options.epsNum, options.epsDen, sigmaLCnstr, addLCnstrIdx);
    }
  ...
  BooleanType & BC_isBound)
{
  ...
  if (constraints.hasNoLower() == BT_FALSE) {
    ...
    performRatioTest(
      nIAC, IAC_idx, &constraints, num, den, options.epsNum, options.epsDen, tau, BC_idx_tmp);
 ...
 */
returnValue QProblem::setupAuxiliaryQP(
  ...
  if (y_min_number >= 0) {
    ...
    if (y_min_isBound) {
      ...
      constraints.setStatus(
        y_min_number, (status_ == ST_LOWER) ? ST_INFEASIBLE_LOWER : ST_INFEASIBLE_UPPER);
      // TODO: fix duals y[]
