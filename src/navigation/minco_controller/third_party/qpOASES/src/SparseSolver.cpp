...
extern "C"
{
   ...
   */
  extern void MA57AD(fint_t * n, /* Order of matrix. */
   ...
   */
  extern void MA57BD(fint_t * n, /* Order of matrix. */
    fint_t * ne,                 /* Number of entries. */
   ...
   * MA57CD -- Solution.
   */
  extern void MA57CD(fint_t * job, /* Solution job.  Solve for... */
                                   /* JOB <= 1:  A */
                                   /* JOB == 2:  PLP^t */
                                   /* JOB == 3:  PDP^t */
                                   /* JOB >= 4:  PL^t P^t */
...
Ma57SparseSolver::Ma57SparseSolver() : SparseSolver()
{
  ...
  factorization (default: 1e-20)
  cntl_ma57[0] = 0.5;			// Set pivot tolerance: Higher values = more stable but slower/less sparse
  (default: 0.01, max 0.5) */
}
...
__attribute__((constructor)) static void MPIinit(void)
{
  int mpi_initialized;
...
__attribute__((destructor)) static void MPIfini(void)
{
  int mpi_finalized;
...
MumpsSparseSolver & MumpsSparseSolver::operator=(const SparseSolver & rhs)
{
  ...
  if (!mumps_rhs) {
    fprintf(getGlobalMessageHandler()->getOutputFile(),
      "Error in MumpsSparseSolver& MumpsSparseSolver::operator=( const SparseSolver& rhs )\n");
    throw; /* TODO: More elegant exit? */
  }
  if (this != mumps_rhs) {
    clear();
...
returnValue MumpsSparseSolver::factorize()
{
  ...
  mumps_data->icntl[12] =
    1;  // avoid lapack bug, ensures proper inertia; mentioned to be very expensive in mumps manual
  ...
  if (error == -8 || error == -9)  // not enough memory
  {
    ...
    for (int trycount = 0; trycount < trycount_max; trycount++) {
      ...
      ComputeMemIncrease(mumps_data->icntl[13],
        2.0 * (double)old_mem_percent,
        MUMPS_INT(0),
        "percent extra working space for MUMPS");
  ...
  if (error == -13) {
    MyPrintf("MUMPS returned INFO(1) =%i - out of memory when trying to allocate % %s.\nIn some cases it "
             "helps to decrease the value of the option \"mumps_mem_percent\".\n",
      error,
      mumps_data->info[1] < 0 ? -mumps_data->info[1] : mumps_data->info[1],
 ...
 *	s o l v e
 */
returnValue MumpsSparseSolver::solve(int_t dim_, const real_t * const rhs, real_t * const sol)
{
