  ...
  BooleanType only_lower_triangular) const
{
  ...
  return getSparseSubmatrix(irows->getLength(),
    rowsNumbers,
    icols->getLength(),
    colsNumbers,
    rowoffset,
...
returnValue SparseMatrix::getRow(
  int_t rNum, const Indexlist * const icols, real_t alpha, real_t * row) const
{
  long i, j, k;
...
returnValue SparseMatrix::getCol(
  int_t cNum, const Indexlist * const irows, real_t alpha, real_t * col) const
{
  long i, j;
    ...
    if (xN == 1) {
      ...
      for (l = 0; l < icols->length; l++) {
        ...
        if (isZero(xmax) == BT_FALSE) {
          ...
          for (i = jc[j]; i < jc[j + 1]; i++)
            for (k = 0; k < xN; k++)
              // AW: Maybe it makes more sense to order ytmp by vectors, not vector entries, for better
              // cache peformance?
...
returnValue SparseMatrixRow::getRow(
  int_t rNum, const Indexlist * const icols, real_t alpha, real_t * row) const
{
  long i, j;
...
returnValue SparseMatrixRow::getCol(
  int_t cNum, const Indexlist * const irows, real_t alpha, real_t * col) const
{
  long i, j, k, srt;
...
returnValue SparseMatrixRow::getSparseSubmatrix(int_t irowsLength,
  const int_t * const irowsNumber,
  int_t icolsLength,
  const int_t * const icolsNumber,
  int_t rowoffset,
