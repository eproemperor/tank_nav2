  ...
  if (options.enableFarBounds == BT_FALSE) {
    ...
    for (;;) {
      ...
      returnvalue = solveRegularisedQP(
        g_new, lb_new_far, ub_new_far, nWSR, &cputime_remaining, nWSR_performed, isFirstCall);
 ...
 */
BooleanType QProblemB::isCPUtimeLimitExceeded(
  const real_t * const cputime, real_t starttime, int_t nWSR) const
{
  ...
  BooleanType isFirstCall)
{
  ...
  for (iter = nWSRperformed; iter < nWSR; ++iter) {
    ...
    returnvalue =
      determineStepDirection(delta_g, delta_lb, delta_ub, Delta_bB_isZero, delta_xFX, delta_xFR, delta_yFX);
    if (returnvalue != SUCCESSFUL_RETURN) {
      delete[] delta_yFX;
      delete[] delta_xFX;
      delete[] delta_xFR;
    ...
    returnvalue =
      performStep(delta_g, delta_lb, delta_ub, delta_xFX, delta_xFR, delta_yFX, BC_idx, BC_status);
    if (returnvalue != SUCCESSFUL_RETURN) {
      delete[] delta_yFX;
      delete[] delta_xFX;
      delete[] delta_xFR;
 ...
 */
returnValue QProblemB::setupAuxiliaryWorkingSet(
  const Bounds * const auxiliaryBounds, BooleanType setupAfresh)
{
  int_t i;
  int_t nV = getNV();
  ...
  real_t * const delta_yFX)
{
  ...
  for (r = 0; r <= options.numRefinementSteps; ++r) {
    ...
    if (nFR > 0) {
      ...
      if ((hessianType != HST_ZERO) && (hessianType != HST_IDENTITY) && (Delta_bB_isZero == BT_FALSE) &&
          (r == 0))
