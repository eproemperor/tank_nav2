...
private:
  ...
  double *
    a_ma27; /**< matrix/factor for MA27 (A in MA27).  If have_factorization is false, it contains the matrix
  ...
  bool have_factorization; /**< flag indicating whether factorization for current matrix has already been
                              computed */
...
template <typename T>
inline void ComputeMemIncrease(T & len,  ///< current length on input, new length on output
...
)
{
  if (recommended >= std::numeric_limits<T>::max()) {
    ...
    if (len < std::numeric_limits<T>::max()) {
      ...
      what << "Cannot allocate more than " << std::numeric_limits<T>::max() * sizeof(T) << " bytes for "
