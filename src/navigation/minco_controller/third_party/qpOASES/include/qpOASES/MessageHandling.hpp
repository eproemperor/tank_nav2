...
enum returnValue
{
  ...
  RET_NO_CHOLESKY_WITH_INITIAL_GUESS, /**< Externally computed Cholesky factor cannot be combined with an
                                         initial guess. */
  ...
  RET_CONSTRAINTS_NOT_SPECIFIED,      /**< Problem does comprise constraints! You also have to specify new
                                         constraints' bounds. */
  ...
  RET_ADDCONSTRAINT_FAILED_INFEASIBILITY, /**< Addition of constraint to working set failed (due to QP
                                             infeasibility). */
  ...
  RET_ZERO_HESSIAN_ASSUMED,       /**< Zero Hessian matrix assumed as null pointer passed without specifying
                                     hessianType. */
  ...
  RET_INERTIA_CORRECTION_FAILED, /**< Inertia correction failed after KKT matrix had too many negative
                                    eigenvalues. */
...
public:
   ...
   *  Errors are defined as abnormal events which cause an immediate termination of the current (sub)
   * function. Errors of a sub function should be commented by the calling function by means of a warning
   * message (if this error does not cause an error of the calling function, either)! \return Error number
  ...
  returnValue throwError(returnValue Enumber, /**< Error number returned by sub function call. */
    ...
    VisibilityStatus localVisibilityStatus /**< Determines (locally) if error message can be printed to
   ...
   *  Warnings are definied as abnormal events which does NOT cause an immediate termination of the current
   * (sub) function. \return Warning number returned by sub function call
   */
  returnValue throwWarning(returnValue Wnumber, /**< Warning number returned by sub function call. */
    ...
    VisibilityStatus localVisibilityStatus /**< Determines (locally) if warning message can be printed to
                                            * stdFile. If GLOBAL visibility status of the message is set to
                                            * VS_HIDDEN, no message is printed, anyway! */
  );
  ...
  returnValue throwInfo(returnValue Inumber, /**< Info number returned by sub function call. */
    ...
    VisibilityStatus localVisibilityStatus   /**< Determines (locally) if info message can be printed to
                                              * stdFile.   If GLOBAL visibility status of the message is set to
                                              * VS_HIDDEN,   no message is printed, anyway! */
  );
