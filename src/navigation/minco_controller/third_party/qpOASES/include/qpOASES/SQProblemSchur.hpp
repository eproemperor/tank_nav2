 ...
 *	\author Andreas Waechter and Dennis Janka, based on QProblem.hpp by Hans Joachim Ferreau, Andreas
...
private:
  ...
  returnValue addToSchurComplement(int_t number,
  ...
  /** Update QR factorization and determinant of Schur complement after a row and column have been added or
   * removed */
  ...
  /** If negative curvature is discovered in the reduced Hessian, add bounds until all eigenvalues are
   * positive */
  returnValue correctInertia();

  /** If the KKT matrix is declared singular during refactorization, remove linearly dependent constraints
   * or add bounds */
  returnValue repairSingularWorkingSet();

  returnValue stepCalcRhs(int_t nFR,
    int_t nFX,
    int_t nAC,
    int_t * FR_idx,
    int_t * FX_idx,
  ...
  returnValue stepCalcReorder(int_t nFR,
    int_t nAC,
    int_t * FR_idx,
    int_t * AC_idx,
    int_t nFRStart,
  ...
  returnValue stepCalcBacksolveSchur(int_t nFR,
    int_t nFX,
    int_t nAC,
    int_t * FR_idx,
    int_t * FX_idx,
  ...
  returnValue stepCalcReorder2(int_t nFR,
    int_t nAC,
    int_t * FR_idx,
    int_t * AC_idx,
    int_t nFRStart,
  ...
  returnValue stepCalcResid(int_t nFR,
    int_t nFX,
    int_t nAC,
    int_t * FR_idx,
    int_t * FX_idx,
  ...
  returnValue stepCalcDeltayFx(int_t nFR,
    int_t nFX,
    int_t nAC,
    int_t * FX_idx,
    const real_t * const delta_g,
...
protected:
  ...
  int_t nS;    /**< Current size of Schur complement matrix. -1 means that the Schur complement has not yet
                  been initialized. */
