  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  nav2_util::declare_parameter_if_not_declared(
    node, name + ".odom_frame", rclcpp::ParameterValue("map"));
  ...
  odom_sub_ = node->create_subscription<nav_msgs::msg::Odometry>("/Odometry",
    odom_qos,
  ...
  cmd_vel_mpc_pub_ = node->create_publisher<geometry_msgs::msg::Twist>("/cmd_vel", 1);   //TODO: 2026/10.1版本，minco规划器的cmd_mpc话题需要验证是否存在
  ...
  nav2_core::GoalChecker * goal_checker)
{
  ...
  // 3) 调用 MPC 求解（输出 global_frame_ 系速度，此处为 map）
  Control u_global;
