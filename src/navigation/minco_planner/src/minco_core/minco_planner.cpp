  ...
  const std::string & planner_mode_param, const std::string & map_frame, const std::string & rog_frame)
{
  ...
  mode_params_.rog_frame = rog_frame.empty() ? "map" : rog_frame;
  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  std::string configured_rog_frame = "map";
  nav2_util::declare_parameter_if_not_declared(
  ...
  global_path_pub_ = node->create_publisher<nav_msgs::msg::Path>(
    name_ + "/global_path", rclcpp::QoS(1).transient_local());
  ...
  const geometry_msgs::msg::PoseStamped & start, const geometry_msgs::msg::PoseStamped & goal)
{
  ...
  // 优先返回 FSM 已算出的真实全局路径快照；为空（首拍）时退回 start+goal 两点。
  bool got_real_path = false;
  {
    std::lock_guard<std::mutex> lk(path_mutex_);
    if (!latest_global_path_.empty()) {
      path.poses = latest_global_path_;
      path.header.stamp = rclcpp::Clock().now();
      path.header.frame_id = output_frame_;
      got_real_path = true;
    }
...
//2026.10.1
void MincoPlanner::publishGlobalPath()
{
    if (!global_path_pub_) {
        return;
    }
