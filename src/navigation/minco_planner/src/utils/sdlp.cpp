...
template <int d> bool sdlp::unit(double * a)
{
  double mag = 0.0;
  for (int i = 0; i <= d; i++) {
    mag += a[i] * a[i];
...
template <int d> int sdlp::lp_no_con(const double * n_vec, const double * d_vec, double * opt)
{
  double n_dot_d = 0.0;
  double d_dot_d = 0.0;
  for (int i = 0; i <= d; i++) {
...
void sdlp::lp_min_lin_rat(const bool degen,
  const double * cw_vec,
  const double * ccw_vec,
  const double * n_vec,
  const double * d_vec,
  double * opt)
{
...
int sdlp::wedge(const double (*halves)[2],
  const int m,
  int * next,
  int * prev,
  double * cw_vec,
...
int sdlp::lp_base_case(const double (*halves)[2],
  const int m,
  const double * n_vec,
  const double * d_vec,
  double * opt,
...
template <int d> void sdlp::findimax(const double * pln, int * imax)
{
  *imax = 0;
...
template <int d>
int sdlp::linfracprog(const double * halves,
  const int max_size,
  const int m,
  const double * n_vec,
  const double * d_vec,
...
template <>
int sdlp::linfracprog<1>(const double * halves,
  const int /*max_size*/,
  const int m,
  const double * n_vec,
  const double * d_vec,
...
template <int d>
double sdlp::linprog(const Eigen::Matrix<double, d, 1> & c,
  ...
  Eigen::Matrix<double, d, 1> & x)
{
  ...
  int status = sdlp::linfracprog<d>(
    halves.data(), m, m, n_vec.data(), d_vec.data(), opt.data(), work.data(), next.data(), prev.data());
...
template double sdlp::linprog<1>(const Eigen::Matrix<double, 1, 1> & c,
  const Eigen::Matrix<double, -1, 1> & A,
  const Eigen::Matrix<double, -1, 1> & b,
  Eigen::Matrix<double, 1, 1> & x);

template double sdlp::linprog<4>(const Eigen::Matrix<double, 4, 1> & c,
  const Eigen::Matrix<double, -1, 4> & A,
  const Eigen::Matrix<double, -1, 1> & b,
