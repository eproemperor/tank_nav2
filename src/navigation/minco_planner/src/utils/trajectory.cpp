...
Trajectory::Trajectory(const std::vector<double> & durs, const std::vector<Eigen::MatrixXd> & cMats)
{
...
void Trajectory::emplace_back(const double & dur, const Eigen::MatrixXd & cMat)
{
...
bool Trajectory::getPartialTrajectoryByID(
  const int & start_id, const int & end_id, Trajectory & out_traj) const
{
  out_traj.clear();
  int end_id_ = end_id;
  if (end_id_ == -1) {
...
bool Trajectory::getPartialTrajectoryByTime(
  const double & start_TT, const double & end_TT, Trajectory & out_traj) const
{
  /*
  ...
  if (end_TT <= 0 || end_TT > total_dur) {
    std::cout << YELLOW << "Partial traj end_t error. [end_TT]: " << end_TT << " [Total Dur]: " << total_dur
              << RESET << std::endl;
    return false;
  }
  if (end_TT <= start_TT) {
    std::cout << YELLOW << "Time duration wrong: start_t: " << start_TT << "; end_t: " << end_TT << RESET
              << std::endl;
    return false;
  }
  ...
  if (pieces[pieceIdx].getDegree() == 5) {
    ...
    cvt_M << 1, 0, 0, 0, 0, 0, 5 * t0, 1, 0, 0, 0, 0, 10 * t02, 4 * t0, 1, 0, 0, 0, 10 * t03, 6 * t02,
      3 * t0, 1, 0, 0, 5 * t04, 4 * t03, 3 * t02, 2 * t0, 1, 0, t05, 1 * t04, 1 * t03, t02, t0, 1;
    ...
    cvt_M << 1, 0, 0, 0, 0, 0, 0, 0, 7 * t0, 1, 0, 0, 0, 0, 0, 0, 21 * t02, 6 * t0, 1, 0, 0, 0, 0, 0,
      35 * t03, 15 * t02, 5 * t0, 1, 0, 0, 0, 0, 35 * t04, 20 * t03, 10 * t02, 4 * t0, 1, 0, 0, 0, 21 * t05,
      15 * t04, 10 * t03, 6 * t02, 3 * t0, 1, 0, 0, 7 * t06, 6 * t05, 5 * t04, 4 * t03, 3 * t02, 2 * t0, 1,
      0, t07, t06, 1 * t05, t04, t03, t02, t0, 1;
...
{
  std::cout << GREEN << "[Trajectory::printProfile] The trajectory has " << getPieceNum() << " pieces"
            << RESET << std::endl;
