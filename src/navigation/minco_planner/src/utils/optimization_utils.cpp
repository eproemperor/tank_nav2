...
bool Gcopter<EIGENVEC>::smoothedL1(const double & x, const double & mu, double & f, double & df)
{
  if (x < 0.0) {
    return false;
  } else if (x > mu) {
...
template <typename EIGENVEC>
void Gcopter<EIGENVEC>::forwardMapTauToT(const Eigen::VectorXd & tau, Eigen::VectorXd & T)
{
  const long sizeTau = tau.size();
  T.resize(sizeTau);
  for (long i = 0; i < sizeTau; i++) {
    T(i) =
      tau(i) > 0.0 ? ((0.5 * tau(i) + 1.0) * tau(i) + 1.0) : 1.0 / ((0.5 * tau(i) - 1.0) * tau(i) + 1.0);
  }
}
...
template <typename EIGENVEC>
void Gcopter<EIGENVEC>::propagateGradientTToTau(
...
template <typename EIGENVEC>
void Gcopter<EIGENVEC>::mapIntervalToInf(
  const double & lower_bound, const double & upper_bound, const double & inter, double & inf)
{
...
template <typename EIGENVEC>
void Gcopter<EIGENVEC>::mapInfToInterval(
  const double & lower_bound, const double & upper_bound, const double & inf, double & inter)
{
...
template <typename EIGENVEC>
void Gcopter<EIGENVEC>::propagateGradIntervalToInf(const double & lower_bound,
...
template <typename EIGENVEC>
void Gcopter<EIGENVEC>::normRetrictionLayer(
  const Eigen::VectorXd & xi, const PolyhedronV & vPoly, double & cost, EIGENVEC & gradXi)
{
...
template <typename EIGENVEC>
void Gcopter<EIGENVEC>::backwardGradP(
  const Eigen::VectorXd & xi, const PolyhedronV & vPoly, const Eigen::Matrix3Xd & gradP, EIGENVEC & gradXi)
{
  const long sizeP = xi.size() / vPoly.cols();
  gradXi.resize(xi.size());
  ...
  for (long i = 0, j = 0, k; i < sizeP; i++, j += k) {
    ...
    gradQ.resize(k);
    gradQ.head(k - 1) =
      (vPoly.rightCols(k - 1).transpose() * gradP.col(i)).array() * unitQ.head(k - 1).array() * 2.0;
    gradQ(k - 1) = 0.0;
  ...
  EIGENVEC & gradXi)
{
  ...
  for (long i = 0, j = 0, k, l; i < sizeP; i++, j += k) {
    ...
    gradQ.resize(k);
    gradQ.head(k - 1) =
      (vPolys[l].rightCols(k - 1).transpose() * gradP.col(i)).array() * unitQ.head(k - 1).array() * 2.0;
    gradQ(k - 1) = 0.0;
    gradXi.segment(j, k) = (gradQ - unitQ * unitQ.dot(gradQ)) * normInv;
  }
}
...
template <typename EIGENVEC>
void Gcopter<EIGENVEC>::forwardP(
  const Eigen::VectorXd & xi, const Eigen::VectorXi & vIdx, const PolyhedraV & vPolys, Eigen::Matrix3Xd & P)
{
  const long sizeP = vIdx.size();
  P.resize(3, sizeP);
  Eigen::VectorXd q;
...
template <typename EIGENVEC>
void Gcopter<EIGENVEC>::forwardP(
  const Eigen::VectorXd & xi, const PolyhedronV & vPoly, Eigen::Matrix3Xd & P)
{
  const long sizeP = xi.size() / vPoly.cols();
  P.resize(3, sizeP);
  Eigen::VectorXd q;
  for (long i = 0, j = 0, k; i < sizeP; i++, j += k) {
    k = vPoly.cols();
    q = xi.segment(j, k).normalized().head(k - 1);
    P.col(i) = vPoly.rightCols(k - 1) * q.cwiseProduct(q) + vPoly.col(0);
  }
}

template <typename EIGENVEC>
void Gcopter<EIGENVEC>::backwardP(
  const Eigen::Matrix3Xd & P, const Eigen::VectorXi & vIdx, const PolyhedraV & vPolys, EIGENVEC & xi)
{
  ...
  for (long i = 0, j = 0, k, l; i < sizeP; i++, j += k) {
    ...
    lbfgs::lbfgs_optimize(
      x, minSqrD, &Gcopter<EIGENVEC>::costTinyNLS, nullptr, nullptr, &ovPoly, tiny_nls_params);
...
template <typename EIGENVEC>
void Gcopter<EIGENVEC>::normRetrictionLayer(const Eigen::VectorXd & xi,
  const Eigen::VectorXi & vIdx,
  const PolyhedraV & vPolys,
  double & cost,
  EIGENVEC & gradXi)
...
template class optimization_utils::Gcopter<Eigen::Map<Eigen::VectorXd>>;