...
void MINCO_S2NU::setConditions(const Eigen::Matrix<double, 3, 2> & headState,
...
void MINCO_S2NU::propogateGrad(const Eigen::MatrixX3d & partialGradByCoeffs,
...
void MINCO_S3NU::setConditions(const Eigen::Matrix<double, 3, 3> & headState,
...
void MINCO_S3NU::setConditions(
  const Eigen::Matrix<double, 3, 3> & headState, const Eigen::Matrix<double, 3, 3> & tailState)
{
  headPVA = headState;
  tailPVA = tailState;
}
...
void MINCO_S3NU::propogateGrad(const Eigen::MatrixX3d & partialGradByCoeffs,
  const Eigen::VectorXd & partialGradByTimes,
  Eigen::Matrix3Xd & gradByPoints,
  Eigen::VectorXd & gradByTimes,
  bool free_end)
{
  ...
  for (int i = 0; i < N - 1; i++) {
    ...
    // negative jerk
    B1.row(5) =
      -(6.0 * b.row(i * 6 + 3) + 24.0 * T1(i) * b.row(i * 6 + 4) + 60.0 * T2(i) * b.row(i * 6 + 5));
...
void MINCO_S3NU::propagateGradOfWayptsAndState(const Eigen::MatrixX3d & partialGradByCoeffs,
  const Eigen::VectorXd & partialGradByTimes,
  Eigen::VectorXd & gradByTimes,
...
void MINCO_S4NU::setConditions(const Eigen::Matrix<double, 3, 4> & headState,
...
void MINCO_S4NU::setConditions(
  const Eigen::Matrix<double, 3, 4> & headState, const Eigen::Matrix<double, 3, 4> & tailState)
{
  headPVAJ = headState;
  tailPVAJ = tailState;
  return;
...
void MINCO_S4NU::getEnergyPartialGradByCoeffs(Eigen::MatrixX3d & gdC) const
{
  ...
  for (int i = 0; i < N; i++) {
    gdC.row(8 * i + 7) = 10080.0 * b.row(8 * i + 4) * T4(i) + 40320.0 * b.row(8 * i + 5) * T5(i) +
                         100800.0 * b.row(8 * i + 6) * T6(i) + 201600.0 * b.row(8 * i + 7) * T7(i);
...
void MINCO_S4NU::propogateGrad(const Eigen::MatrixX3d & partialGradByCoeffs,
  const Eigen::VectorXd & partialGradByTimes,
  Eigen::Matrix3Xd & gradByPoints,
  Eigen::VectorXd & gradByTimes,
  bool free_end)
{
  ...
  for (int i = 0; i < N - 1; i++) {
    ...
    // negative jerk
    B1.row(6) =
      -(6.0 * b.row(i * 8 + 3) + 24.0 * T1(i) * b.row(i * 8 + 4) + 60.0 * T2(i) * b.row(i * 8 + 5) +
