...
Ellipsoid::Ellipsoid(const Mat3f & R, const Vec3f & r, const Vec3f & d) : R_(R), r_(r), d_(d)
{
  undefined = false;
...
double Ellipsoid::pointDistaceToEllipsoid(const Vec3f & pt, Vec3f & closest_pt_on_ellip) const
{
  ...
  double dist = geometry_utils::DistancePointEllipsoid(r_(0),
    r_(1),
    r_(2),
    pt_ellip_frame.x(),
    pt_ellip_frame.y(),
...
bool Ellipsoid::noPointsInside(
  vec_Vec3f & pc, const Eigen::Matrix3d & R, const Vec3f & r, const Vec3f & p) const
{
