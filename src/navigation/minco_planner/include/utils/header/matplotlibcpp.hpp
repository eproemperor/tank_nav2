  ...
  const std::map<std::string, std::string> & keywords)
{
  ...
  PyObject * kwargs = PyDict_New();
  for (std::map<std::string, std::string>::const_iterator it = keywords.begin(); it != keywords.end();
       ++it) {
...
template <typename T> struct is_callable_impl<true, T>
{
  ...
  template <typename U>
  static std::true_type test(...);  // use a variadic function to make sure (1) it accepts everything and
                                    // (2) its always the worst match
...
{
  // dispatch to is_callable_impl<true, T> or is_callable_impl<false, T> depending on whether T is of class
  // type or not
