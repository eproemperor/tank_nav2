...
template <typename T> class BinaryFileHandler
{
public:
  ...
  static void save(const std::string & filename, const T & data)
  {
    ...
    archive(data);
    std::cout << COLOR_SUCCESS << "Data saved successfully to " << filename << COLOR_RESET << '\n';
  }
  ...
  static void load(const std::string & filename, T & data)
  {
    ...
    archive(data);
    std::cout << COLOR_SUCCESS << "Data loaded successfully from " << filename << COLOR_RESET << '\n';
  }
};
