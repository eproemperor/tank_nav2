...
SlidingMap::SlidingMap(const Vec3i & half_map_size_i,
...
void SlidingMap::initSlidingMap(const rog_map::Vec3i & half_map_size_i,
  const double & resolution,
  const bool & map_sliding_en,
  const double & sliding_thresh,
...
void SlidingMap::printMapInformation()
{
  ...
  std::cout << GREEN << "\tlocal_map_size_d: " << sc_.map_size_i.cast<double>().transpose() * sc_.resolution
            << RESET << std::endl;
}
