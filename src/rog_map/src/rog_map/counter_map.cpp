  ...
  const double & unk_thresh)
{
  ...
  /* 1) compute the resolution ratio, ensure the resolution is an integer (for CORNER) or odd numbered
   * multiple (for CENTER)
   * */
  ...
  initSlidingMap(
    half_counter_map_size_i, counter_map_resolution, map_sliding_en, sliding_thresh, fix_map_origin);
